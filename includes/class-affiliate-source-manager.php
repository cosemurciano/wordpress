...
class ALMA_Affiliate_Source_Manager {
    ...
    private function maybe_handle_source_form(){ if(($_SERVER['REQUEST_METHOD']??'')!=='POST'||($_POST['action_type']??'')!=='save_source')return; if(!wp_verify_nonce($_POST['alma_source_nonce']??'','alma_save_source'))wp_die('Nonce non valido'); if(!current_user_can('manage_options'))wp_die('Unauthorized'); global $wpdb;
        ...
        if(!in_array($provider_preset, array('viator','getyourguide'), true)){ foreach((array)($_POST['credentials_extra_fields']??array()) as $k=>$v){ $k=sanitize_key($k); $v=sanitize_text_field(wp_unslash($v)); if($v!=='')$credentials[$k]=$v; }}
    ...
    private function render_import_result_page(){
        ...
        echo '<ul><li>Selezionati: '.intval($data['selected']).'</li><li>Creati: '.intval($r['created']??0).'</li><li>Aggiornati: '.intval($r['updated']??0).'</li><li>Saltati: '.intval($r['skipped']??0).'</li><li>Errori: '.intval($r['errors']??0).'</li><li>Immagini importate: '.intval($r['image_imported']??0).'</li><li>Immagini riutilizzate: '.intval($r['image_reused']??0).'</li><li>Immagini non importate: '.intval($r['image_skipped']??0).'</li><li>Warning immagini: '.intval($r['image_failed']??0).'</li></ul>';
