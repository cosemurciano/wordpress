...
class ALMA_Affiliate_Source_Manager {
    ...
    public function render_sources_page(){ if(!current_user_can('manage_options')) wp_die('Unauthorized'); $this->maybe_ensure_sources_table(); settings_errors('alma_source'); global $wpdb; $terms=get_terms(array('taxonomy'=>'link_type','hide_empty'=>false)); if(is_wp_error($terms)||!is_array($terms))$terms=array(); $presets=$this->get_provider_presets();
        ...
        $ai_instructions = isset($es['ai_source_instructions']) ? (string) $es['ai_source_instructions'] : ALMA_Affiliate_Link_AI_Context_Builder::DEFAULT_INSTRUCTIONS;
        $ai_refresh = isset($es['ai_context_refresh_interval']) ? (string) $es['ai_context_refresh_interval'] : '7d';
        $api_sync = isset($es['api_sync_interval']) ? (string) $es['api_sync_interval'] : 'manual';
        $regen_policy = isset($es['ai_context_regeneration_policy']) ? (string) $es['ai_context_regeneration_policy'] : 'if_hash_changed_or_expired';

        echo '<div class="alma-section"><h3>Istruzioni e aggiornamento AI</h3><p><label><strong>Istruzioni AI per questa Source</strong><br/><textarea name="settings_fields[ai_source_instructions]" class="large-text" rows="6">'.esc_textarea($ai_instructions).'</textarea></label></p><p class="description">Regole generali che l’Agente AI deve rispettare quando usa i contenuti provenienti da questa Source.</p><p><label><strong>AI Context refresh interval</strong><br/><select name="settings_fields[ai_context_refresh_interval]">'.$this->render_options(array('manual'=>'manual','24h'=>'24h','72h'=>'72h','7d'=>'7d','30d'=>'30d'),$ai_refresh).'</select></label></p><p><label><strong>API Sync interval</strong><br/><select name="settings_fields[api_sync_interval]">'.$this->render_options(array('manual'=>'manual','24h'=>'24h','72h'=>'72h','7d'=>'7d','30d'=>'30d'),$api_sync).'</select></label></p><p><label><strong>AI context regeneration policy</strong><br/><select name="settings_fields[ai_context_regeneration_policy]">'.$this->render_options(array('only_if_hash_changed'=>'only_if_hash_changed','if_hash_changed_or_expired'=>'if_hash_changed_or_expired','always_on_import'=>'always_on_import','manual_only'=>'manual_only'),$regen_policy).'</select></label></p><p class="description">Questi valori sono salvati nelle impostazioni Source e usati dal builder del contesto AI interno.</p></div><div class="alma-section"><h3>Configurazione provider</h3><div id="alma-guided-settings"></div></div><div class="alma-section"><h3>Credenziali</h3><div id="alma-guided-credentials"></div>';
    ...
    private function render_import_contents_page(){
        ...
        echo '<div class="wrap"><h1>Importa contenuti</h1><p><a class="button" href="'.esc_url($list_url).'">Torna alla lista Sources</a></p>';
        ...
        echo '<ul><li><strong>Nome Source:</strong> '.esc_html($source['name']).'</li><li><strong>Provider:</strong> '.esc_html(($source['provider_label']?:$source['provider'])).'</li><li><strong>Preset:</strong> '.esc_html($source['provider_preset']?:'—').'</li><li><strong>Stato:</strong> '.(((int)$source['is_active']===1)?'Attivo':'Disattivo').'</li><li><strong>Import limit massimo:</strong> 100</li><li><strong>Duplicate policy:</strong> '.esc_html($settings['duplicate_policy']??'skip_existing').'</li><li><strong>Regenerate AI context on import:</strong> '.(((string)($settings['regenerate_ai_context_on_import']??'1')==='1')?'Sì':'No').'</li></ul>';
        echo '<div class="notice notice-info"><p>Questa importazione userà le Tipologie Link configurate nella Source. I prodotti già importati vengono riconosciuti tramite ID esterno provider. Prezzo, rating e destinazione non vengono salvati come campi separati: saranno usati nel Contesto AI. Importa prima 1 elemento per testare.</p></div>';
        if(empty($term_ids)){ echo '<div class="notice notice-warning"><p>Nessuna Tipologia Link configurata nella Source. Configura prima di importare.</p></div>'; }
        $load_preview = isset($_GET['load_preview']) && $_GET['load_preview']==='1';
        $base = add_query_arg(array('post_type'=>'affiliate_link','page'=>'alma-affiliate-sources','alma_view'=>'import_contents','source_id'=>$source_id),admin_url('edit.php'));
        echo '<form method="get"><input type="hidden" name="post_type" value="affiliate_link"/><input type="hidden" name="page" value="alma-affiliate-sources"/><input type="hidden" name="alma_view" value="import_contents"/><input type="hidden" name="source_id" value="'.(int)$source_id.'"/><input type="hidden" name="load_preview" value="1"/><p><label>Modalità ricerca <select name="import_search_model"><option value="freetext_search">freetext_search</option><option value="products_search">products_search</option></select></label> <label>Keyword <input type="text" name="import_search_term" placeholder="es. Lecce"/></label> <label>Destination ID Viator <input type="text" name="import_destination_id"/></label> <label>Quantità <input type="number" name="import_limit" min="1" max="100" value="10"/></label></p><p class="description">Il filtro periodo disponibilità riguarda quando l’esperienza è prenotabile, non quando è stata pubblicata. Per prodotti nuovi usa il flag NEW_ON_VIATOR o ordina per DATE_ADDED.</p><p><button class="button button-primary">Carica anteprima</button></p></form>';
        if($load_preview){
            $criteria=array('import_search_model'=>sanitize_key($_GET['import_search_model']??'freetext_search'),'import_search_term'=>sanitize_text_field(wp_unslash($_GET['import_search_term']??'')),'import_destination_id'=>sanitize_text_field(wp_unslash($_GET['import_destination_id']??'')),'import_limit'=>max(1,min(100,(int)($_GET['import_limit']??10)))); $preview_service=new ALMA_Affiliate_Source_Import_Preview_Service(); $items=$preview_service->get_preview_items($source,$criteria);
            if(is_wp_error($items)){ echo '<div class="notice notice-error"><p>'.esc_html($items->get_error_message()).'</p></div></div>'; return; }
            $ext_ids=array(); foreach((array)$items as $it){ $eid=(string)($it['productCode']??$it['external_id']??''); if($eid!=='')$ext_ids[]=$eid; }
            $existing=$preview_service->find_existing_map((int)$source_id,$ext_ids); $dup=sanitize_key($settings['duplicate_policy']??'skip_existing');
            echo '<form method="post">'; wp_nonce_field('alma_import_selected','alma_import_selected_nonce');
            ...
            echo '<p><button type="button" class="button alma-select-all">Seleziona tutti</button> <button type="button" class="button alma-deselect-all">Deseleziona tutti</button> <span class="alma-selected-counter">0 selezionati</span></p>';
            echo '<table class="widefat striped alma-import-preview"><thead><tr><th></th><th>Stato</th><th>Titolo</th><th>External ID</th><th>URL affiliato</th><th>Origine URL</th><th>Warning</th><th>Azione prevista</th></tr></thead><tbody>';
            foreach((array)$items as $it){ $v=is_array($it['_alma_validation']??null)?$it['_alma_validation']:array(); $eid=(string)($v['external_id']??($it['productCode']??$it['external_id']??'')); if($eid==='')continue; $exists=!empty($existing[$eid]); $checked=(!empty($v['selectable_default']) && ((!$exists)||$dup==='update_existing')); if($exists&&$dup==='skip_existing')$checked=false; $st=$exists?'già importato':'nuovo'; if(($v['status']??'')==='error'){$st='errore';$checked=false;} $action=$exists?($dup==='update_existing'?'aggiorna':'salta'):'crea'; if(($v['status']??'')==='error')$action='salta';
                $warn=implode(' | ',array_merge((array)($v['errors']??array()),(array)($v['warnings']??array())));
                echo '<tr><td><input class="alma-select-item" type="checkbox" name="selected_external_ids[]" value="'.esc_attr($eid).'" '.checked($checked,true,false).'></td><td>'.esc_html($st).'</td><td>'.esc_html($it['title']??$it['name']??'—').'</td><td>'.esc_html($eid).'</td><td>'.esc_html(($v['has_affiliate_url']??'no')==='yes'?'Sì':'No').'</td><td>'.esc_html($v['url_origin']??'productUrl').'</td><td>'.esc_html($warn!==''?$warn:'—').'</td><td>'.esc_html($action).'</td></tr>'; }
            echo '</tbody></table><p><button class="button button-primary" '.(empty($term_ids)?'disabled':'').'>Importa selezionati</button></p></form>';
        }
        echo '</div>';
    }
    private function handle_import_selected_items(){
        ...
        if(!empty($source['deleted_at'])){ $result = new WP_Error('source_archived', __('Source archiviata: import bloccato.', 'affiliate-link-manager-ai')); } else {
            $criteria=array('import_search_model'=>sanitize_key($_POST['import_search_model']??'freetext_search'),'import_search_term'=>sanitize_text_field(wp_unslash($_POST['import_search_term']??'')),'import_destination_id'=>sanitize_text_field(wp_unslash($_POST['import_destination_id']??'')),'import_limit'=>max(1,min(100,(int)($_POST['import_limit']??10))));
