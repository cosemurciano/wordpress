...
class ALMA_Affiliate_Source_Manager {
    ...
    public function render_importable_fields_page(){
        ...
        $fields=(array)($result['fields']??array()); $catalog = ALMA_Affiliate_Source_Viator_Field_Catalog::get_catalog();
        $detected_paths = array(); foreach($fields as $f){ $detected_paths[(string)($f['path']??'')] = true; }
        ...
        foreach($catalog as $c){ $status = !empty($detected_paths[$c['path']]) ? 'rilevato nel campione' : (($c['status']==='protected')?'protetto/compliance':'documentato ma non rilevato'); echo '<tr><td>'.esc_html($c['path']).'</td><td>'.esc_html($c['group']).'</td><td>'.esc_html($c['origin']).'</td><td>'.esc_html($c['type']).'</td><td>'.esc_html($c['description']).(!empty($c['note'])?' — '.$c['note']:'').'</td><td>'.esc_html($c['example']).'</td><td>'.esc_html($c['mapping_hint']).'</td><td>'.esc_html($status).'</td></tr>'; }
        echo '</tbody></table></div>';
    }
