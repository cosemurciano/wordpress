...
class ALMA_Affiliate_Source_Manager {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_alma_gyg_csv_save_activity_mapping', array($this, 'ajax_gyg_csv_save_activity_mapping'));
        add_action('wp_ajax_alma_gyg_csv_start_activity_import', array($this, 'ajax_gyg_csv_start_activity_import'));
        add_action('wp_ajax_alma_gyg_csv_process_manual_batch', array($this, 'ajax_gyg_csv_process_manual_batch'));
        add_action('wp_ajax_alma_gyg_csv_get_activity_progress', array($this, 'ajax_gyg_csv_get_activity_progress'));
        add_action('wp_ajax_alma_gyg_csv_get_session_logs', array($this, 'ajax_gyg_csv_get_session_logs'));
        add_action('admin_post_alma_gyg_csv_download_session_file', array($this, 'handle_gyg_csv_download_session_file'));
        add_action('admin_post_alma_gyg_csv_download_report_csv', array($this, 'handle_gyg_csv_download_report_csv'));
        add_action('admin_post_alma_gyg_csv_download_log_json', array($this, 'handle_gyg_csv_download_log_json'));
    ...
    public static function create_tables() { global $wpdb; require_once ABSPATH.'wp-admin/includes/upgrade.php'; $c=$wpdb->get_charset_collate();
        ...
        ALMA_Affiliate_Source_GYG_CSV_Import_Job_Service::create_log_table();
    }
    ...
    public function handle_sources_page_load(){
        ...
        if($action==='gyg_csv_import_type'){ $this->handle_gyg_csv_import_type(); return; }
        if($action==='gyg_csv_simple_import'){ $this->handle_gyg_csv_import_type(); return; }
    ...
    private function render_import_contents_page(){
        ...
        if($load_preview){
            ...
            $load_more_url=add_query_arg(array_merge($_GET,array('post_type'=>'affiliate_link','page'=>'alma-affiliate-sources','criteria_token'=>$criteria_token,'load_preview'=>'1','import_start'=>$next_start)),admin_url('edit.php'));
    ...
    private function render_gyg_csv_import_page($source){
        ...
        echo '<div class="notice notice-info"><p><strong>GetYourGuide CSV / Deep Link:</strong> importa CSV locali senza chiamate esterne. Il nuovo flusso è manuale: scegli il batch size e clicca Importa prossimo batch quando vuoi processare i record successivi.</p></div>';
        ...
        echo '<div class="postbox"><h2 class="hndle"><span>GetYourGuide CSV / Deep Link</span></h2><div class="inside"><p>Associa ogni <strong>Tipologia attività CSV</strong> a una o più <strong>Tipologie Link affiliati</strong>. L’importazione non usa più job background o auto-processing: ogni click processa un solo batch e aggiorna il report.</p></div></div>';
        ...
        $mappings=is_array($settings['type_mappings']??null)?$settings['type_mappings']:array(); $progress_rows=$svc->get_progress_for_session(absint($session['id']??0)); $jobs=new ALMA_Affiliate_Source_GYG_CSV_Import_Job_Service(); $job_rows=$jobs->get_latest_jobs_for_session(absint($session['id']??0));
        $terms=get_terms(array('taxonomy'=>$this->get_gyg_link_type_taxonomy(),'hide_empty'=>false)); if(is_wp_error($terms)||!is_array($terms)) $terms=array();
        $source_fallback=ALMA_Affiliate_Source_GYG_CSV_Importer::normalize_mapping_term_ids(json_decode((string)($source['destination_term_ids']??''),true)); if(empty($source_fallback)&&!empty($source['destination_term_id'])) $source_fallback=array(absint($source['destination_term_id']));
        $highlight=sanitize_text_field(wp_unslash($_GET['activity_type_hash']??''));
        echo '<div class="postbox alma-gyg-activity-summary" data-source-id="'.(int)$source_id.'" data-session-id="'.absint($session['id']??0).'" data-token="'.esc_attr($token).'"><h2 class="hndle"><span>Riepilogo tipologie attività</span></h2><div class="inside"><table class="widefat striped"><thead><tr><th>Tipologia attività CSV</th><th>Record totali</th><th>Mapping Sothra</th><th>Progressi</th><th>Azione</th></tr></thead><tbody>';
        foreach((array)($summary['types']??array()) as $type=>$count){
            ...
            $total=absint($count); $processed=absint($progress['last_cursor']??0); $status=$processed>0?'partial':'ready'; if($processed>=$total&&$total>0)$status='completed'; if($job && in_array(sanitize_key($job['status']), array('queued','running','paused'), true)) $status='partial'; $percent=$total>0?min(100,round(($processed/$total)*100,1)):0;
            $counts=array('imported'=>absint($progress['imported_count']??0),'updated'=>absint($progress['updated_count']??0),'existing'=>absint($progress['existing_count']??0),'skipped'=>absint($progress['skipped_count']??0),'errors'=>absint($progress['error_count']??0));
            ...
            echo '<td><label class="description" for="alma-gyg-batch-size-'.esc_attr($hash).'">Batch size</label><br/><select id="alma-gyg-batch-size-'.esc_attr($hash).'" class="alma-gyg-batch-size"><option value="25">25</option><option value="50" selected>50</option><option value="100">100</option><option value="250">250</option></select><br/><button type="button" class="button button-primary alma-gyg-start-activity-import">Importa prossimo batch</button><br/><button type="button" class="button alma-gyg-retry-errors" disabled>Riprova errori</button></td></tr>';
        }
        echo '</tbody></table><p class="description">Totale righe: '.(int)($summary['total']??0).' · URL non validi: '.(int)($summary['invalid_urls']??0).' · record senza città: '.(int)($summary['without_city']??0).' · record senza regione: '.(int)($summary['without_region']??0).'</p>';
        $report_url=wp_nonce_url(admin_url('admin-post.php?action=alma_gyg_csv_download_report_csv&session_id='.absint($session['id']??0).'&source_id='.(int)$source_id), 'alma_gyg_csv_report_'.absint($session['id']??0));
        $log_url=wp_nonce_url(admin_url('admin-post.php?action=alma_gyg_csv_download_log_json&session_id='.absint($session['id']??0).'&source_id='.(int)$source_id), 'alma_gyg_csv_log_'.absint($session['id']??0));
        $back_url=add_query_arg(array('post_type'=>'affiliate_link','page'=>'alma-affiliate-sources','alma_view'=>'import_contents','source_id'=>(int)$source_id), admin_url('edit.php'));
        echo '<p><a class="button" href="'.esc_url($report_url).'">Scarica report CSV</a> <a class="button" href="'.esc_url($log_url).'">Scarica log JSON</a> <a class="button" href="'.esc_url($back_url).'">Torna alla selezione file</a></p></div></div>';
        $legacy_running=false; foreach($job_rows as $legacy_job){ if(in_array(sanitize_key($legacy_job['status']??''), array('queued','running','paused'), true)){ $legacy_running=true; break; } }
        if($legacy_running) echo '<div class="notice notice-warning"><p><strong>Vecchio job background rilevato.</strong> Il nuovo import manuale non lo usa più. Se lo stato resta bloccato, usa Reset import nella lista sessioni e ricarica il CSV.</p></div>';
        $this->render_gyg_csv_session_logs(absint($session['id']??0), $source_id);
    }

    private function redirect_legacy_gyg_csv_import_type() {
        $args = array('post_type'=>'affiliate_link','page'=>'alma-affiliate-sources','alma_view'=>'import_contents','source_id'=>absint($_GET['source_id'] ?? 0));
        $token = sanitize_key($_GET['gyg_csv_token'] ?? ($_GET['token'] ?? ''));
        $hash = sanitize_text_field(wp_unslash($_GET['activity_type_hash'] ?? ''));
        if ($token !== '') $args['gyg_csv_token'] = $token;
        if ($hash !== '') $args['activity_type_hash'] = $hash;
    ...
    private function gyg_csv_status_label($status) {
        $map = array(
            ...
            'processing'=>__('In elaborazione', 'affiliate-link-manager-ai'),
            'partial'=>__('Parziale', 'affiliate-link-manager-ai'),
        );
    ...
    private function render_gyg_csv_session_logs($session_id, $source_id) {
        ...
        $logs = $jobs->get_logs($session_id, $source_id, '', 80);
        echo '<div class="postbox alma-gyg-session-logs" data-session-id="'.absint($session_id).'" data-source-id="'.absint($source_id).'"><h2 class="hndle"><span>Log importazione</span></h2><div class="inside"><div class="alma-gyg-log"><table class="widefat striped"><thead><tr><th>Data/ora</th><th>Tipologia CSV</th><th>Evento</th><th>Messaggio</th></tr></thead><tbody>';
        if (empty($logs)) echo '<tr><td colspan="4">Nessun evento registrato per questa sessione.</td></tr>';
        foreach ($logs as $log) echo '<tr><td>'.esc_html($log['created_at']).'</td><td>'.esc_html($log['activity_type']).'</td><td>'.esc_html($log['event']).'</td><td>'.esc_html($log['message']).'</td></tr>';
    ...
    private function render_gyg_csv_recent_sessions($source_id, $active_token = ''){
        ...
        foreach($sessions as $row){
            ...
            $totals=$wpdb->get_row($wpdb->prepare("SELECT SUM(last_cursor) processed, SUM(imported_count) imported, SUM(updated_count) updated, SUM(existing_count) existing, SUM(skipped_count) skipped, SUM(error_count) errors, MAX(updated_at) updated_at FROM {$wpdb->prefix}alma_gyg_csv_import_progress WHERE session_id=%d", absint($row['id'])), ARRAY_A);
            $processed=absint($totals['processed']??0);
            $imported=absint($totals['imported']??0)+absint($totals['updated']??0)+absint($totals['existing']??0);
            $remaining=max(0, absint($row['total_rows'])-$processed);
            $status=$processed>0?'partial':(string)$row['status']; if($processed>=absint($row['total_rows'])&&absint($row['total_rows'])>0)$status='completed';
            ...
            echo '<input type="hidden" name="action_type" value="gyg_csv_delete_session"/><input type="hidden" name="source_id" value="'.(int)$source_id.'"/><input type="hidden" name="session_id" value="'.(int)$row['id'].'"/><input type="hidden" name="gyg_csv_token" value="'.esc_attr($row['token']).'"/><button class="button button-small" type="submit">Reset import</button></form></td></tr>';
        }
        echo '</tbody></table><p class="description">Reset import elimina solo sessione, progressi e file CSV, non i Link affiliati già importati.</p></div></div>';
    }

    private function handle_gyg_csv_delete_session(){
        if(!current_user_can('manage_options')) wp_die('Unauthorized');
        global $wpdb; $source_id=absint($_POST['source_id']??0); $session_id=absint($_POST['session_id']??0); $token=sanitize_key($_POST['gyg_csv_token']??'');
        if(!wp_verify_nonce($_POST['alma_gyg_csv_delete_nonce']??'','alma_gyg_csv_delete_session_'.$session_id)) wp_die('Nonce non valido');
        ...
        $svc=new ALMA_Affiliate_Source_GYG_CSV_Importer();
        $res=$svc->delete_session($token,$source_id);
        $args=array('post_type'=>'affiliate_link','page'=>'alma-affiliate-sources','alma_view'=>'import_contents','source_id'=>$source_id);
        if(is_wp_error($res)) $args['gyg_delete_error']=$res->get_error_code(); else $args['gyg_session_deleted']=1;
    ...
    private function handle_gyg_csv_import_selected(){
        ...
        global $wpdb; $source_id=absint($_POST['source_id']??0); $token=sanitize_key($_POST['gyg_csv_token']??''); $type=sanitize_text_field(wp_unslash($_POST['gyg_activity_type']??'')); $term_id=absint($_POST['link_type_term_id']??0); $selected=array_values(array_unique(array_filter(array_map('sanitize_text_field',(array)($_POST['selected_external_ids']??array()))))); $update_existing=isset($_POST['update_existing']) && (string)$_POST['update_existing']==='1';
    ...
    private function render_import_result_page(){
        ...
        if(isset($r['imported']) || isset($r['already_present']) || isset($r['invalid_urls'])){ echo '<ul><li>Selezionati: '.intval($data['selected']).'</li><li>Record processati: '.intval($r['processed']??0).'</li><li>Importati: '.intval($r['imported']??0).'</li><li>Aggiornati: '.intval($r['updated']??0).'</li><li>Già presenti saltati: '.intval($r['already_present']??($r['existing']??0)).'</li><li>Match deduplica non validi/stale: '.intval($r['dedupe_matches_stale']??0).'</li><li>Saltati: '.intval($r['skipped']??0).'</li><li>Errori: '.intval($r['errors']??0).'</li><li>URL non validi: '.intval($r['invalid_urls']??0).'</li><li>Titoli letti da Titolo Attività: '.intval($r['titles_read']??($r['titles_populated']??0)).'</li><li>Titoli salvati nei Link affiliati: '.intval($r['titles_saved']??0).'</li><li>Contesti AI popolati: '.intval($r['ai_contexts_populated']??0).'</li><li>Contesti AI saltati per policy manual_only: '.intval($r['ai_contexts_skipped_manual_only']??0).'</li><li>Record senza città: '.intval($r['without_city']??0).'</li><li>Record senza regione: '.intval($r['without_region']??0).'</li><li>Durata batch: '.esc_html((string)($r['duration']??0)).'s</li></ul>'; } else { echo '<ul><li>Selezionati: '.intval($data['selected']).'</li><li>Creati: '.intval($r['created']??0).'</li><li>Aggiornati: '.intval($r['updated']??0).'</li><li>Saltati: '.intval($r['skipped']??0).'</li><li>Errori: '.intval($r['errors']??0).'</li><li>Immagini importate: '.intval($r['image_imported']??0).'</li><li>Immagini riutilizzate: '.intval($r['image_reused']??0).'</li><li>Immagini non importate: '.intval($r['image_skipped']??0).'</li><li>Warning immagini: '.intval($r['image_failed']??0).'</li></ul>'; }
    ...
    private function render_gyg_csv_import_type_page() {
        $ctx = $this->get_gyg_csv_simple_context_from_request('GET');
        $source_id = absint($_GET['source_id'] ?? 0);
        $token = sanitize_key($_GET['gyg_csv_token'] ?? '');
        $activity_hash = sanitize_text_field(wp_unslash($_GET['activity_type_hash'] ?? ''));
        $back = add_query_arg(array('post_type'=>'affiliate_link','page'=>'alma-affiliate-sources','alma_view'=>'import_contents','source_id'=>$source_id,'gyg_csv_token'=>$token), admin_url('edit.php'));
        $self_url = add_query_arg(array('post_type'=>'affiliate_link','page'=>'alma-affiliate-sources','alma_view'=>'import_contents','source_id'=>$source_id,'gyg_csv_token'=>$token,'activity_type_hash'=>$activity_hash), admin_url('edit.php'));
        $list_url = add_query_arg(array('post_type'=>'affiliate_link','alma_source_filter'=>$source_id), admin_url('edit.php'));

        echo '<div class="wrap"><h1>Importa tipologia GetYourGuide CSV</h1>';
        echo '<p><a class="button" href="'.esc_url($back).'">Torna al riepilogo tipologie</a> <a class="button" href="'.esc_url($list_url).'">Vai alla lista Link affiliati filtrata per source</a></p>';
        ...
        echo '<tr><th>Nome file</th><td>'.esc_html($ctx['session']['name'] ?? 'CSV').'</td></tr>';
        echo '<tr><th>File CSV</th><td>'.esc_html($ctx['session']['stored_filename'] ?? '').'</td></tr>';
        echo '<tr><th>Totale righe</th><td>'.absint($ctx['session']['total_rows'] ?? ($ctx['summary']['total'] ?? 0)).'</td></tr>';
        echo '<tr><th>Data caricamento</th><td>'.esc_html($ctx['session']['created_at'] ?? '').'</td></tr>';
        echo '<tr><th>Source</th><td>'.esc_html($ctx['source']['name'] ?? '').' (#'.(int)$ctx['source_id'].')</td></tr>';
        echo '<tr><th>Partner ID</th><td>'.esc_html($settings['partner_id'] ?? '').'</td></tr>';
        echo '<tr><th>UTM medium</th><td>'.esc_html($settings['utm_medium'] ?? 'online_publisher').'</td></tr>';
        echo '</tbody></table></div></div>';

        echo '<div class="postbox"><h2 class="hndle"><span>Tipologia attività</span></h2><div class="inside"><table class="widefat striped"><tbody>';
        echo '<tr><th>Nome tipologia CSV</th><td>'.esc_html($ctx['activity_type']).'</td></tr>';
        echo '<tr><th>Record totali</th><td>'.absint($counts['total'] ?? 0).'</td></tr>';
        echo '<tr><th>Importati</th><td>'.absint($progress['imported_count'] ?? 0).'</td></tr>';
        echo '<tr><th>Aggiornati</th><td>'.absint($progress['updated_count'] ?? 0).'</td></tr>';
        echo '<tr><th>Già presenti</th><td>'.absint($progress['existing_count'] ?? 0).'</td></tr>';
        echo '<tr><th>Saltati</th><td>'.absint($progress['skipped_count'] ?? 0).'</td></tr>';
        echo '<tr><th>Errori</th><td>'.absint($progress['error_count'] ?? 0).'</td></tr>';
        echo '<tr><th>Da importare</th><td>'.absint($counts['remaining'] ?? 0).'</td></tr>';
        echo '</tbody></table></div></div>';

        $filter = $preview['filters'];
        $filter_base = array('post_type'=>'affiliate_link','page'=>'alma-affiliate-sources','alma_view'=>'import_contents','source_id'=>$ctx['source_id'],'gyg_csv_token'=>$ctx['token'],'activity_type_hash'=>$ctx['activity_type_hash']);
    ...
    private function render_gyg_csv_import_type_report($report, $back_url, $self_url, $list_url) {
        echo '<div class="postbox"><h2 class="hndle"><span>Report ultimo import</span></h2><div class="inside"><table class="widefat striped"><tbody>';
        $labels = array('selected'=>'Record selezionati','processed'=>'Record processati','imported'=>'Record importati','updated'=>'Record aggiornati','existing'=>'Già presenti saltati','dedupe_matches_stale'=>'Match deduplica non validi/stale','skipped'=>'Record saltati','errors'=>'Errori','invalid_urls'=>'URL non validi','titles_read'=>'Titoli letti da Titolo Attività','titles_saved'=>'Titoli salvati nei Link affiliati','titles_missing'=>'Record senza Titolo Attività','ai_contexts_populated'=>'Contesti AI popolati','ai_contexts_skipped_manual_only'=>'Contesti AI saltati per policy manual_only','ai_contexts_missing'=>'Contesti AI non popolati','link_types_associated'=>'Tipologie Link associate','without_city'=>'Record senza città','without_region'=>'Record senza regione','requested'=>'Quantità richiesta','effective_processed'=>'Quantità effettivamente processata');
        foreach ($labels as $key=>$label) echo '<tr><th>'.esc_html($label).'</th><td>'.absint($report[$key] ?? 0).'</td></tr>';
        echo '</tbody></table>';
        if (!empty($report['logs'])) { echo '<h3>Log</h3><ul>'; foreach (array_slice((array)$report['logs'], 0, 20) as $log) echo '<li>'.esc_html($log).'</li>'; echo '</ul>'; }
        echo '<p><a class="button" href="'.esc_url($back_url).'">Torna al riepilogo tipologie</a> <a class="button button-primary" href="'.esc_url($self_url).'">Importa altri record di questa tipologia</a> <a class="button" href="'.esc_url($list_url).'">Vai alla lista Link affiliati filtrata per source</a></p>';
        echo '</div></div>';
    }

    private function handle_gyg_csv_import_type() {
        if (!current_user_can('manage_options')) wp_die('Unauthorized');
        if (!wp_verify_nonce($_POST['alma_gyg_csv_import_type_nonce'] ?? ($_POST['alma_gyg_csv_simple_nonce'] ?? ''), 'alma_gyg_csv_import_type')) wp_die('Nonce non valido');
        $ctx = $this->get_gyg_csv_simple_context_from_request('POST');
        $redirect_base = array('post_type'=>'affiliate_link','page'=>'alma-affiliate-sources','alma_view'=>'import_contents','source_id'=>absint($_POST['source_id'] ?? 0),'gyg_csv_token'=>sanitize_key($_POST['gyg_csv_token'] ?? ''),'activity_type_hash'=>sanitize_text_field(wp_unslash($_POST['activity_type_hash'] ?? '')));
        if (is_wp_error($ctx)) { wp_safe_redirect(add_query_arg(array_merge($redirect_base, array('alma_gyg_error'=>$ctx->get_error_code())), admin_url('edit.php'))); exit; }
        $taxonomy = $this->get_gyg_link_type_taxonomy();
    ...
    private function get_valid_gyg_ajax_context() {
        ...
        global $wpdb;
        $source_id=absint($_POST['source_id']??0); $token=sanitize_key($_POST['token']??''); $type=sanitize_text_field(wp_unslash($_POST['activity_type']??'')); $activity_hash=sanitize_text_field(wp_unslash($_POST['activity_type_hash']??''));
        $source=$wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}alma_affiliate_sources WHERE id=%d",$source_id),ARRAY_A);
        if(!is_array($source) || sanitize_key($source['provider_preset']??'')!=='gyg_csv') return new WP_Error('invalid_source',__('Source gyg_csv non valida.', 'affiliate-link-manager-ai'));
        ...
        $summary=is_array($session['summary']??null)?$session['summary']:$svc->summarize($session['path'],$det['columns']);
        return array('source_id'=>$source_id,'token'=>$token,'activity_type'=>$type,'activity_type_hash'=>ALMA_Affiliate_Source_GYG_CSV_Importer::activity_type_hash($type),'source'=>$source,'svc'=>$svc,'session'=>$session,'columns'=>$det['columns'],'summary'=>$summary);
    }
    ...
    public function ajax_gyg_csv_prepare_import(){
        ...
        $progress_payload=array('imported_count'=>absint($progress['imported_count']??0),'updated_count'=>absint($progress['updated_count']??0),'existing_count'=>absint($progress['existing_count']??0),'skipped_count'=>absint($progress['skipped_count']??0),'error_count'=>absint($progress['error_count']??0),'last_cursor'=>absint($progress['last_cursor']??0),'updated_at'=>(string)($progress['updated_at']??''));
        wp_send_json_success(array('activity_type'=>$ctx['activity_type'],'activity_type_hash'=>$ctx['activity_type_hash'],'source_id'=>$ctx['source_id'],'token'=>$ctx['token'],'source_name'=>$ctx['source']['name']??'','source_active'=>!empty($ctx['source']['is_active']),'partner_id'=>$settings['partner_id']??'','utm_medium'=>$settings['utm_medium']??'online_publisher','counts'=>$counts,'terms'=>$term_data,'mapped_term_ids'=>$mapped,'preview'=>$preview,'progress'=>$progress_payload,'max_quantity'=>ALMA_Affiliate_Source_GYG_CSV_Importer::MAX_IMPORT_QUANTITY));
    }
    ...
    public function ajax_gyg_csv_import_batch(){
        ...
        $ctx['svc']->upsert_progress(absint($ctx['session']['id']??0),$ctx['source_id'],$ctx['activity_type'],$term_ids,$result);
        $names=array(); foreach($term_ids as $tid){ $term=get_term($tid,$taxonomy); if($term&&!is_wp_error($term)) $names[]=$term->name; }
        $result['mapping_label']=implode(', ',$names); $result['activity_type_hash']=$ctx['activity_type_hash']; $result['progress']=$ctx['svc']->get_progress(absint($ctx['session']['id']??0),$ctx['source_id'],$ctx['activity_type']); $result['max_quantity']=ALMA_Affiliate_Source_GYG_CSV_Importer::MAX_IMPORT_QUANTITY;
    ...
    private function gyg_csv_validate_activity_mapping_terms($raw) {
        $taxonomy = $this->get_gyg_link_type_taxonomy();
        $term_ids = ALMA_Affiliate_Source_GYG_CSV_Importer::normalize_mapping_term_ids($raw);
        foreach ($term_ids as $tid) {
            $term = get_term($tid, $taxonomy);
            if (!$term || is_wp_error($term)) return new WP_Error('invalid_term', __('Tipologia Sothra non valida.', 'affiliate-link-manager-ai'));
        }
        return $term_ids;
    }

    private function gyg_csv_activity_mapping_context_from_ajax() {
        $ctx = $this->get_valid_gyg_ajax_context();
        if (is_wp_error($ctx)) return $ctx;
        if (absint($ctx['session']['id'] ?? 0) <= 0) return new WP_Error('invalid_session', __('Sessione CSV non valida.', 'affiliate-link-manager-ai'));
        return $ctx;
    }

    private function gyg_csv_save_activity_mapping_for_context($ctx, $term_ids, $event_message = '') {
        global $wpdb;
        $settings = ALMA_Affiliate_Source_GYG_CSV_Importer::default_settings($this->decode_db_json($ctx['source']['settings'] ?? '{}'));
        $settings['type_mappings'][$ctx['activity_type']] = $term_ids;
        $wpdb->update("{$wpdb->prefix}alma_affiliate_sources", array('settings'=>wp_json_encode($settings),'updated_at'=>current_time('mysql')), array('id'=>$ctx['source_id']));
        $ctx['svc']->upsert_progress(absint($ctx['session']['id'] ?? 0), $ctx['source_id'], $ctx['activity_type'], $term_ids, array('done'=>true));
        $jobs = new ALMA_Affiliate_Source_GYG_CSV_Import_Job_Service();
        $jobs->log_event(absint($ctx['session']['id'] ?? 0), $ctx['source_id'], $ctx['activity_type'], 'mapping salvato', $event_message !== '' ? $event_message : __('Mapping Sothra salvato per la tipologia CSV.', 'affiliate-link-manager-ai'));
    }

    public function ajax_gyg_csv_save_activity_mapping() {
        $ctx = $this->gyg_csv_activity_mapping_context_from_ajax(); if (is_wp_error($ctx)) $this->send_gyg_ajax_context_error($ctx);
        $term_ids = $this->gyg_csv_validate_activity_mapping_terms($_POST['term_ids'] ?? array()); if (is_wp_error($term_ids)) wp_send_json_error(array('message'=>$term_ids->get_error_message(),'code'=>$term_ids->get_error_code()),400);
        $this->gyg_csv_save_activity_mapping_for_context($ctx, $term_ids);
        wp_send_json_success(array('message'=>__('Mapping salvato.', 'affiliate-link-manager-ai'),'session_id'=>absint($ctx['session']['id']),'source_id'=>$ctx['source_id'],'activity_type_hash'=>$ctx['activity_type_hash'],'status'=>'saved'));
    }

    public function ajax_gyg_csv_start_activity_import() {
        $ctx = $this->gyg_csv_activity_mapping_context_from_ajax(); if (is_wp_error($ctx)) $this->send_gyg_ajax_context_error($ctx);
        $term_ids = $this->gyg_csv_validate_activity_mapping_terms($_POST['term_ids'] ?? array()); if (is_wp_error($term_ids)) wp_send_json_error(array('message'=>$term_ids->get_error_message(),'code'=>$term_ids->get_error_code()),400);
        if (empty($term_ids)) wp_send_json_error(array('message'=>__('Seleziona almeno una Tipologia Link affiliati per questa tipologia CSV.', 'affiliate-link-manager-ai'),'code'=>'missing_terms'),400);
        $this->gyg_csv_save_activity_mapping_for_context($ctx, $term_ids, __('Mapping salvato prima dell’avvio importazione.', 'affiliate-link-manager-ai'));
        $jobs = new ALMA_Affiliate_Source_GYG_CSV_Import_Job_Service();
        $existing = $jobs->get_latest_job_for_activity(absint($ctx['session']['id']), $ctx['source_id'], $ctx['activity_type_hash']);
        if (!empty($existing) && in_array((string)$existing['status'], array('queued','running','paused'), true)) {
            $jobs->log_event(absint($ctx['session']['id']), $ctx['source_id'], $ctx['activity_type'], 'job ripreso', __('Job esistente ripreso per questa tipologia CSV.', 'affiliate-link-manager-ai'));
            $status = $jobs->run_batch(absint($existing['id']));
        } else {
            $criteria = array('activity_type'=>$ctx['activity_type'], 'activity_type_hash'=>$ctx['activity_type_hash'], 'session_token'=>$ctx['token']);
            $job_id = $jobs->create_job($ctx['session'], $ctx['source'], $ctx['activity_type'], $criteria, array(), array(), $term_ids, !empty($_POST['update_existing']));
    ...
    private function gyg_csv_manual_batch_size_from_request() {
        $raw = isset($_POST['batch_size']) ? wp_unslash($_POST['batch_size']) : 50;
        if (!is_numeric($raw)) return new WP_Error('invalid_batch_size', __('Batch size non valido. Scegli 25, 50, 100 o 250.', 'affiliate-link-manager-ai'));
        $size = absint($raw);
        if ($size < 1) return new WP_Error('invalid_batch_size', __('Batch size non valido. Inserisci un numero positivo.', 'affiliate-link-manager-ai'));
        if ($size < 10 || $size > 250) return new WP_Error('invalid_batch_size', __('Batch size fuori limite. Usa un valore tra 10 e 250 record.', 'affiliate-link-manager-ai'));
        return $size;
    }

    private function gyg_csv_manual_status_payload($ctx, $batch_result = array(), $message = '') {
        $progress = $ctx['svc']->get_progress(absint($ctx['session']['id'] ?? 0), $ctx['source_id'], $ctx['activity_type']);
        $total = absint($ctx['summary']['types'][$ctx['activity_type']] ?? 0);
        if ($total < 1) {
            $counts = $ctx['svc']->count_existing_for_type($ctx['session']['path'], $ctx['columns'], $ctx['activity_type'], $ctx['source']);
            $total = absint($counts['total'] ?? 0);
        }
        $processed = min($total, absint($progress['last_cursor'] ?? 0));
        $remaining = max(0, $total - $processed);
        $status = $processed > 0 ? 'partial' : 'ready';
        if ($total > 0 && $remaining === 0) $status = 'completed';
        if (is_array($batch_result) && isset($batch_result['status_override'])) $status = sanitize_key($batch_result['status_override']);
        $percent = $total > 0 ? min(100, round(($processed / $total) * 100, 1)) : 0;
        $last_report = json_decode((string)($progress['last_report_json'] ?? ''), true);
        if (!is_array($last_report)) $last_report = array();
        return $this->gyg_csv_normalize_status_payload(array(
            'session_id'=>absint($ctx['session']['id'] ?? 0),
            'source_id'=>$ctx['source_id'],
            'activity_type_hash'=>$ctx['activity_type_hash'],
            'status'=>$status,
            'total_records'=>$total,
            'processed_records'=>$processed,
            'remaining_records'=>$remaining,
            'imported_count'=>absint($progress['imported_count'] ?? 0),
            'updated_count'=>absint($progress['updated_count'] ?? 0),
            'existing_count'=>absint($progress['existing_count'] ?? 0),
            'skipped_count'=>absint($progress['skipped_count'] ?? 0),
            'error_count'=>absint($progress['error_count'] ?? 0),
            'invalid_urls_count'=>absint($last_report['invalid_urls'] ?? 0),
            'incomplete_records_count'=>absint($last_report['incomplete_records'] ?? 0),
            'already_present_count'=>absint($last_report['already_present'] ?? ($last_report['existing'] ?? 0)),
            'unprocessed_records'=>$remaining,
            'percent'=>$percent,
            'message'=>$message !== '' ? $message : ($remaining > 0 ? __('Batch completato. Puoi importare il prossimo batch quando vuoi.', 'affiliate-link-manager-ai') : __('Importazione completata.', 'affiliate-link-manager-ai')),
            'updated_at'=>(string)($progress['updated_at'] ?? ''),
            'batch_report'=>is_array($batch_result) ? $batch_result : array(),
            'last_report'=>$last_report,
        ));
    }

    public function ajax_gyg_csv_process_manual_batch() {
        $ctx = $this->gyg_csv_activity_mapping_context_from_ajax(); if (is_wp_error($ctx)) $this->send_gyg_ajax_context_error($ctx);
        $batch_size = $this->gyg_csv_manual_batch_size_from_request(); if (is_wp_error($batch_size)) wp_send_json_error(array('message'=>$batch_size->get_error_message(),'code'=>$batch_size->get_error_code()),400);
        $term_ids = $this->gyg_csv_validate_activity_mapping_terms($_POST['term_ids'] ?? array()); if (is_wp_error($term_ids)) wp_send_json_error(array('message'=>$term_ids->get_error_message(),'code'=>$term_ids->get_error_code()),400);
        if (empty($term_ids)) wp_send_json_error(array('message'=>__('Seleziona almeno una Tipologia Link affiliati per questa tipologia CSV.', 'affiliate-link-manager-ai'),'code'=>'missing_terms'),400);
        $lock_key = 'alma_gyg_csv_manual_lock_' . absint($ctx['session']['id']) . '_' . $ctx['activity_type_hash'];
        if (get_transient($lock_key)) wp_send_json_error(array('message'=>__('Un batch è già in corso per questa tipologia. Attendi il completamento e riprova.', 'affiliate-link-manager-ai'),'code'=>'batch_locked'),409);
        set_transient($lock_key, 1, 5 * MINUTE_IN_SECONDS);
        $jobs = new ALMA_Affiliate_Source_GYG_CSV_Import_Job_Service();
        try {
            $this->gyg_csv_save_activity_mapping_for_context($ctx, $term_ids, __('Mapping salvato prima del batch manuale.', 'affiliate-link-manager-ai'));
            $progress = $ctx['svc']->get_progress(absint($ctx['session']['id']), $ctx['source_id'], $ctx['activity_type']);
            $cursor = absint($progress['last_cursor'] ?? 0);
            $total = absint($ctx['summary']['types'][$ctx['activity_type']] ?? 0);
            $jobs->log_event(absint($ctx['session']['id']), $ctx['source_id'], $ctx['activity_type'], 'batch manuale avviato', sprintf(__('Batch manuale avviato: size %1$d, offset %2$d.', 'affiliate-link-manager-ai'), $batch_size, $cursor), array('batch_size'=>$batch_size,'offset'=>$cursor));
            $result = $ctx['svc']->import_batch($ctx['session']['path'], $ctx['columns'], $ctx['activity_type'], $ctx['source'], $term_ids, max($total, $cursor + $batch_size), $cursor, !empty($_POST['update_existing']), $batch_size);
            if (is_wp_error($result)) throw new Exception($result->get_error_message());
            $result['batch_size'] = $batch_size;
            $result['range'] = array('from'=>$cursor + 1, 'to'=>absint($result['next_cursor']));
            $result['unprocessed'] = max(0, $total - absint($result['next_cursor']));
            $ctx['svc']->upsert_progress(absint($ctx['session']['id']), $ctx['source_id'], $ctx['activity_type'], $term_ids, $result);
            $message = !empty($result['done']) ? __('Batch completato. Se non restano record, l’importazione è completa.', 'affiliate-link-manager-ai') : __('Batch completato. Controlla il report e clicca di nuovo per continuare.', 'affiliate-link-manager-ai');
            $jobs->log_event(absint($ctx['session']['id']), $ctx['source_id'], $ctx['activity_type'], 'batch manuale completato', $message, $result);
            $payload = $this->gyg_csv_manual_status_payload($ctx, $result, $message);
            $payload['logs'] = $jobs->get_logs(absint($ctx['session']['id']), $ctx['source_id'], $ctx['activity_type_hash'], 30);
            delete_transient($lock_key);
            wp_send_json_success($payload);
        } catch (Exception $e) {
            delete_transient($lock_key);
            global $wpdb;
            $wpdb->update($wpdb->prefix.'alma_gyg_csv_import_sessions', array('status'=>'failed','last_error'=>sanitize_text_field($e->getMessage()),'updated_at'=>current_time('mysql')), array('id'=>absint($ctx['session']['id'])));
            $jobs->log_event(absint($ctx['session']['id']), $ctx['source_id'], $ctx['activity_type'], 'batch manuale fallito', __('Batch interrotto. Controlla il CSV o scarica il log JSON.', 'affiliate-link-manager-ai'), array('error'=>$e->getMessage()));
            wp_send_json_error(array('message'=>__('Il batch non è stato completato. Controlla i dati e riprova; il pulsante è stato riabilitato.', 'affiliate-link-manager-ai'),'code'=>'batch_failed'),500);
        }
    }

    public function ajax_gyg_csv_get_activity_progress() {
        $ctx = $this->gyg_csv_activity_mapping_context_from_ajax(); if (is_wp_error($ctx)) $this->send_gyg_ajax_context_error($ctx);
        $jobs = new ALMA_Affiliate_Source_GYG_CSV_Import_Job_Service();
        $status = $this->gyg_csv_manual_status_payload($ctx, array(), __('Stato import manuale aggiornato.', 'affiliate-link-manager-ai'));
        $status['logs'] = $jobs->get_logs(absint($ctx['session']['id']), $ctx['source_id'], $ctx['activity_type_hash'], 30);
        wp_send_json_success($status);
    }

    public function ajax_gyg_csv_get_session_logs() {
        ...
        $source_id = absint($_POST['source_id'] ?? 0); $session_id = absint($_POST['session_id'] ?? 0);
        global $wpdb;
        $belongs = (int)$wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$wpdb->prefix}alma_gyg_csv_import_sessions WHERE id=%d AND source_id=%d", $session_id, $source_id));
        if ($belongs < 1) wp_send_json_error(array('message'=>__('Sessione CSV non valida per questa source.', 'affiliate-link-manager-ai'),'code'=>'invalid_session'),404);
        $jobs = new ALMA_Affiliate_Source_GYG_CSV_Import_Job_Service();
        wp_send_json_success(array('message'=>__('Log importazione aggiornati.', 'affiliate-link-manager-ai'),'session_id'=>$session_id,'source_id'=>$source_id,'logs'=>$jobs->get_logs($session_id, $source_id, '', 80)));
    }
    ...
    private function gyg_csv_validate_session_download($session_id, $source_id, $nonce_action) {
        if (!current_user_can('manage_options')) wp_die(__('Permessi insufficienti.', 'affiliate-link-manager-ai'));
        $session_id = absint($session_id); $source_id = absint($source_id);
        if (!wp_verify_nonce($_GET['_wpnonce'] ?? '', $nonce_action.$session_id)) wp_die(__('Nonce non valido.', 'affiliate-link-manager-ai'));
        global $wpdb;
        $row = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}alma_gyg_csv_import_sessions WHERE id=%d AND source_id=%d", $session_id, $source_id), ARRAY_A);
        if (!is_array($row)) wp_die(__('Sessione import non valida.', 'affiliate-link-manager-ai'));
        return $row;
    }

    public function handle_gyg_csv_download_report_csv() {
        $session_id = absint($_GET['session_id'] ?? 0); $source_id = absint($_GET['source_id'] ?? 0);
        $row = $this->gyg_csv_validate_session_download($session_id, $source_id, 'alma_gyg_csv_report_');
        global $wpdb;
        $progress = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->prefix}alma_gyg_csv_import_progress WHERE session_id=%d AND source_id=%d ORDER BY updated_at DESC", $session_id, $source_id), ARRAY_A);
        nocache_headers();
        header('Content-Type: text/csv; charset=utf-8');
        header('Content-Disposition: attachment; filename="alma-import-report-'.absint($session_id).'.csv"');
        $out = fopen('php://output', 'w');
        fputcsv($out, array('session_id','file','activity_type','status','processed','imported','updated','skipped','duplicates','invalid_urls','incomplete_records','errors','remaining','updated_at'));
        $summary = json_decode((string)($row['summary_json'] ?? ''), true); if (!is_array($summary)) $summary = array('types'=>array());
        foreach ((array)$progress as $pr) {
            $report = json_decode((string)($pr['last_report_json'] ?? ''), true); if (!is_array($report)) $report = array();
            $total = absint($summary['types'][$pr['activity_type']] ?? 0); $processed = absint($pr['last_cursor'] ?? 0); $remaining = max(0, $total - $processed); $status = ($total > 0 && $remaining === 0) ? 'completed' : ($processed > 0 ? 'partial' : 'ready');
            fputcsv($out, array($session_id, sanitize_file_name($row['original_filename'] ?: 'CSV'), $pr['activity_type'], $status, $processed, absint($pr['imported_count']), absint($pr['updated_count']), absint($pr['skipped_count']), absint($pr['existing_count']), absint($report['invalid_urls'] ?? 0), absint($report['incomplete_records'] ?? 0), absint($pr['error_count']), $remaining, $pr['updated_at']));
        }
        fclose($out); exit;
    }

    public function handle_gyg_csv_download_log_json() {
        $session_id = absint($_GET['session_id'] ?? 0); $source_id = absint($_GET['source_id'] ?? 0);
        $row = $this->gyg_csv_validate_session_download($session_id, $source_id, 'alma_gyg_csv_log_');
        $jobs = new ALMA_Affiliate_Source_GYG_CSV_Import_Job_Service();
        $payload = array('generated_at'=>current_time('mysql'),'session'=>array('id'=>$session_id,'source_id'=>$source_id,'file'=>sanitize_file_name($row['original_filename'] ?: 'CSV'),'status'=>sanitize_key($row['status'] ?? 'ready'),'created_at'=>$row['created_at'] ?? '','updated_at'=>$row['updated_at'] ?? ''),'logs'=>$jobs->get_logs($session_id, $source_id, '', 200));
        nocache_headers();
        header('Content-Type: application/json; charset=utf-8');
        header('Content-Disposition: attachment; filename="alma-import-log-'.absint($session_id).'.json"');
