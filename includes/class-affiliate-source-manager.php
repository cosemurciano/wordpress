...
class ALMA_Affiliate_Source_Manager {
    ...
    private function render_import_contents_page(){
        ...
        if($load_preview){
            ...
            $dup=sanitize_key($settings['duplicate_policy']??'skip_existing'); $dedupe_map=$preview_service->build_dedupe_map($source,$items,$dup);
            ...
            foreach((array)$items as $it){ $v=is_array($it['_alma_validation']??null)?$it['_alma_validation']:array(); $eid=(string)($v['external_id']??($it['productCode']??$it['external_id']??'')); if($eid==='')continue; $m=is_array($dedupe_map[$eid]??null)?$dedupe_map[$eid]:array(); $exists=!empty($m['post_id']); $checked=(!empty($v['selectable_default']) && ((!$exists)||$dup==='update_existing')); if($exists&&$dup==='skip_existing')$checked=false; $st=$exists?($m['match_label']??'già importato'):'nuovo'; if(($v['status']??'')==='error'){$st='errore';$checked=false;} $action=$exists?($dup==='update_existing'?'aggiorna':'salta'):'crea'; if(($v['status']??'')==='error')$action='salta';
