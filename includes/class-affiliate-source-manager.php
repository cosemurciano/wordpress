...
class ALMA_Affiliate_Source_Manager {
    ...
    private function render_gyg_csv_import_page($source){
        $source_id=(int)$source['id']; $settings=ALMA_Affiliate_Source_GYG_CSV_Importer::default_settings($this->decode_db_json($source['settings']??'{}')); $svc=new ALMA_Affiliate_Source_GYG_CSV_Importer(); $token=sanitize_key($_GET['gyg_csv_token']??''); $selected_type=sanitize_text_field(wp_unslash($_GET['gyg_activity_type']??''));
        echo '<div class="notice notice-info"><p><strong>GetYourGuide CSV / Deep Link:</strong> importa CSV locali senza chiamate esterne. Il link originale viene conservato e i domini .com/.it non vengono convertiti.</p></div>';
        echo '<div class="postbox"><h2 class="hndle"><span>Step 1 — Caricamento CSV</span></h2><div class="inside"><form method="post" enctype="multipart/form-data">'; wp_nonce_field('alma_gyg_csv_upload','alma_gyg_csv_nonce'); echo '<input type="hidden" name="action_type" value="gyg_csv_upload"/><input type="hidden" name="source_id" value="'.(int)$source_id.'"/><p><input type="file" name="gyg_csv_file" accept=".csv,text/csv" required/> <button class="button button-primary">Carica CSV</button></p><p class="description">Colonne obbligatorie: URL, Tipologia attività, Descrizione attività. Opzionali: Città, Regione di appartenenza. Batch massimo '.(int)$settings['batch_size'].' (limite assoluto 500).</p></form></div></div>';
        if($token==='') return;
        ...
        $summary=$svc->summarize($session['path'],$det['columns']); $mappings=is_array($settings['type_mappings']??null)?$settings['type_mappings']:array(); $terms=get_terms(array('taxonomy'=>'link_type','hide_empty'=>false)); if(is_wp_error($terms)||!is_array($terms))$terms=array();
        ...
        foreach($summary['types'] as $type=>$count){ $mapped=absint($mappings[$type]??0); $term=$mapped?get_term($mapped,'link_type'):null; $url=add_query_arg(array('gyg_csv_token'=>$token,'gyg_activity_type'=>rawurlencode($type)), add_query_arg(array('post_type'=>'affiliate_link','page'=>'alma-affiliate-sources','alma_view'=>'import_contents','source_id'=>$source_id),admin_url('edit.php'))); echo '<tr><td>'.esc_html($type).'</td><td>'.(int)$count.'</td><td>'.esc_html(($term&&!is_wp_error($term))?$term->name:'—').'</td><td><a class="button" href="'.esc_url($url).'">Importa questa tipologia</a></td></tr>'; }
        ...
        $mapped=absint($mappings[$selected_type]??0);
        echo '<div class="postbox"><h2 class="hndle"><span>Step 4 — Mapping tipologia</span></h2><div class="inside"><form method="post">'; wp_nonce_field('alma_gyg_csv_mapping','alma_gyg_csv_mapping_nonce'); echo '<input type="hidden" name="action_type" value="gyg_csv_save_mapping"/><input type="hidden" name="source_id" value="'.(int)$source_id.'"/><input type="hidden" name="gyg_csv_token" value="'.esc_attr($token).'"/><input type="hidden" name="gyg_activity_type" value="'.esc_attr($selected_type).'"/><p><strong>Tipologia CSV:</strong> '.esc_html($selected_type).'</p><p><label>Tipologia Link Sothra<br/><select name="link_type_term_id" required><option value="">— seleziona —</option>'; foreach($terms as $t){ echo '<option value="'.(int)$t->term_id.'"'.selected($mapped,(int)$t->term_id,false).'>'.esc_html($t->name).'</option>'; } echo '</select></label> <button class="button button-primary">Salva mapping</button></p></form></div></div>';
        $filters=array('city'=>sanitize_text_field(wp_unslash($_GET['gyg_city']??'')),'region'=>sanitize_text_field(wp_unslash($_GET['gyg_region']??'')),'search'=>sanitize_text_field(wp_unslash($_GET['gyg_search']??'')),'show_existing'=>isset($_GET['gyg_show_existing']));
        echo '<div class="postbox"><h2 class="hndle"><span>Step 5 — Anteprima record filtrati</span></h2><div class="inside"><form method="get" class="alma-import-grid"><input type="hidden" name="post_type" value="affiliate_link"/><input type="hidden" name="page" value="alma-affiliate-sources"/><input type="hidden" name="alma_view" value="import_contents"/><input type="hidden" name="source_id" value="'.(int)$source_id.'"/><input type="hidden" name="gyg_csv_token" value="'.esc_attr($token).'"/><input type="hidden" name="gyg_activity_type" value="'.esc_attr($selected_type).'"/><p><label>Filtro città<br/><input type="text" name="gyg_city" value="'.esc_attr($filters['city']).'"></label></p><p><label>Filtro regione<br/><input type="text" name="gyg_region" value="'.esc_attr($filters['region']).'"></label></p><p><label>Ricerca libera<br/><input type="text" name="gyg_search" value="'.esc_attr($filters['search']).'"></label></p><p><label><input type="checkbox" name="gyg_show_existing" value="1" '.checked($filters['show_existing'],true,false).'> Mostra anche già importati</label></p><p><button class="button">Filtra anteprima</button></p></form>';
        if($mapped<1){ echo '<div class="notice notice-warning inline"><p>Mapping mancante: seleziona una Tipologia Link Sothra prima di importare.</p></div>'; }
        $items=$svc->preview($session['path'],$det['columns'],$selected_type,$source,$filters);
        echo '<form method="post">'; wp_nonce_field('alma_gyg_csv_import','alma_gyg_csv_import_nonce'); echo '<input type="hidden" name="action_type" value="gyg_csv_import_selected"/><input type="hidden" name="source_id" value="'.(int)$source_id.'"/><input type="hidden" name="gyg_csv_token" value="'.esc_attr($token).'"/><input type="hidden" name="gyg_activity_type" value="'.esc_attr($selected_type).'"/><input type="hidden" name="link_type_term_id" value="'.(int)$mapped.'"/><p><button type="button" class="button alma-select-all">Seleziona tutti i record visibili</button> <button type="button" class="button alma-deselect-all">Deseleziona tutti</button> <span class="alma-selected-counter">0 selezionati</span></p><table class="widefat striped alma-import-preview"><thead><tr><th></th><th>Stato</th><th>URL originale</th><th>URL affiliato</th><th>Città</th><th>Regione</th><th>Tipologia CSV</th><th>Tipologia Sothra</th><th>Descrizione</th><th>Link</th></tr></thead><tbody>';
        if(empty($items)){ echo '<tr><td colspan="10">Nessun record disponibile per questi filtri.</td></tr>'; }
        $term_label='—'; if($mapped){ $term=get_term($mapped,'link_type'); if($term&&!is_wp_error($term))$term_label=$term->name; }
        foreach($items as $it){ echo '<tr'.($it['post_id']?' class="alma-row-existing"':'').'><td><input class="alma-select-item" type="checkbox" name="selected_external_ids[]" value="'.esc_attr($it['external_id']).'" '.disabled($mapped<1,true,false).'></td><td>'.esc_html($it['status']).'</td><td><code>'.esc_html($it['original_url']).'</code></td><td><code>'.esc_html($it['affiliate_url']).'</code></td><td>'.esc_html($it['city']?:'—').'</td><td>'.esc_html($it['region']?:'—').'</td><td>'.esc_html($it['activity_type']).'</td><td>'.esc_html($term_label).'</td><td>'.esc_html($it['description']).'</td><td><a href="'.esc_url($it['original_url']).'" target="_blank" rel="noopener noreferrer">Apri originale</a><br/><a href="'.esc_url($it['affiliate_url']).'" target="_blank" rel="noopener noreferrer">Apri affiliato</a></td></tr>'; }
        echo '</tbody></table><p><button class="button button-primary" '.disabled($mapped<1,true,false).'>Step 6 — Importa selezionati</button></p><p class="description">La selezione/import è limitata a massimo 500 record per batch.</p></form></div></div>';
    }
    ...
    private function handle_gyg_csv_upload(){
        ...
        $source_id=absint($_POST['source_id']??0); $svc=new ALMA_Affiliate_Source_GYG_CSV_Importer(); $res=$svc->handle_upload($_FILES['gyg_csv_file']??array(),$source_id);
    ...
    private function handle_gyg_csv_save_mapping(){
        ...
        $settings=ALMA_Affiliate_Source_GYG_CSV_Importer::default_settings($this->decode_db_json($source['settings']??'{}')); $settings['type_mappings'][$type]=$term_id; $wpdb->update("{$wpdb->prefix}alma_affiliate_sources",array('settings'=>wp_json_encode($settings),'updated_at'=>current_time('mysql')),array('id'=>$source_id));
    ...
    private function handle_gyg_csv_import_selected(){
        ...
        if(count($selected)>500){ $selected=array_slice($selected,0,500); }
