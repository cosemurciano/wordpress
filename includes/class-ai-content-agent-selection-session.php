...
class ALMA_AI_Content_Agent_Selection_Session {
    ...
    public static function add_search_results($payload, $search_results) {
        ...
        $session['instruction_snapshot'] = ALMA_AI_Content_Agent_Instructions_Manager::sanitize_profile_textarea($payload['instruction_snapshot'] ?? '');
    ...
    public static function persist_to_idea($idea_id) {
        ...
        $snapshot = ALMA_AI_Content_Agent_Instructions_Manager::sanitize_profile_textarea($session['instruction_snapshot'] ?? '');
        if ($snapshot === '') { $snapshot = ALMA_AI_Content_Agent_Instructions_Manager::sanitize_profile_textarea($idea['instruction_snapshot'] ?? ''); }
    ...
    public static function load_from_idea($idea) {
        ...
        $session['instruction_snapshot'] = ALMA_AI_Content_Agent_Instructions_Manager::sanitize_profile_textarea($idea['instruction_snapshot'] ?? '');
        $session['updated_at'] = current_time('mysql');
    ...
    public static function build_context_package() {
        ...
        return array('last_query'=>$s['last_query'],'query_history'=>$s['query_history'],'selected_results'=>$selected,'counts'=>$s['counts'],'updated_at'=>$s['updated_at'],'instruction_profile_id'=>$s['instruction_profile_id'],'instruction_profile_name'=>sanitize_text_field($s['instruction_profile_name'] ?? ''),'instruction_snapshot_hash'=>sanitize_text_field($s['instruction_snapshot_hash'] ?? ''),'instruction_snapshot'=>ALMA_AI_Content_Agent_Instructions_Manager::sanitize_profile_textarea($s['instruction_snapshot'] ?? ''),'openai_prompt'=>sanitize_textarea_field($s['openai_prompt'] ?? ''));
    }
}
