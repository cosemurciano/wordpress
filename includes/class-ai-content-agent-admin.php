...
class ALMA_AI_Content_Agent_Admin {
    ...
    private static function render_documents_tab() { global $wpdb; echo '<h2>Documenti TXT</h2><div class="alma-agent-card"><p><strong>Upload TXT</strong> (Disponibile nella prossima fase)</p><p><input type="text" class="regular-text" placeholder="Nome documento"> <input type="file" accept=".txt" disabled> <button class="button" disabled>Carica documento TXT</button></p></div>'; self::action_form('index_document','Indicizza attachment esistente','<input type="number" name="attachment_id" placeholder="Attachment ID" required> <input type="text" name="language_code" placeholder="Lingua (it/en)"> <input type="text" name="usage_mode" value="knowledge">');
        if (self::is_table_missing('knowledge_items')) { echo '<p><em>Tabella knowledge_items mancante: elenco documenti non disponibile.</em></p>'; return; }
        ...
        echo '<h3>Documenti caricati</h3><table class="widefat"><thead><tr><th>Nome</th><th>File</th><th>Stato</th><th>Ultima indicizzazione</th><th>Azioni</th></tr></thead><tbody>';
        foreach ($rows as $r) { echo '<tr><td>'.esc_html($r['title']).'</td><td>'.esc_html($r['source_type']).'</td><td><span class="alma-badge is-pending">pending</span></td><td>'.esc_html($r['indexed_at']).'</td><td><button class="button button-small" disabled>Modifica</button> <button class="button button-small" disabled>Disabilita</button> <button class="button button-small" disabled>Elimina</button></td></tr>'; }
        echo '</tbody></table>';
    }
    private static function render_sources_tab(){ global $wpdb; echo '<h2>Fonti online AI</h2>'; self::action_form('save_source','Salva fonte','<input type="text" name="name" placeholder="Nome fonte" required> <input type="url" name="source_url" placeholder="URL" required><select name="source_type"><option>RSS</option><option>Atom</option><option>Sitemap XML</option><option>Pagina web singola</option><option>Sito web</option><option>Blog/News site</option><option>API JSON</option><option>API XML</option><option>File TXT remoto</option><option>File CSV/TSV remoto</option><option>Podcast RSS</option><option>YouTube/Video</option><option>Newsletter archive</option><option>Social/Web profile</option><option>Altro</option></select><input type="text" name="language_code" placeholder="Tecnologia/lingua"><input type="text" name="market" placeholder="Stato"><input type="text" name="usage_mode" value="knowledge"><label><input type="checkbox" name="is_active" value="1" checked> Attiva</label><textarea name="notes" class="large-text" rows="2" placeholder="Note"></textarea>');
        ...
        foreach($rows as $r){ echo '<tr><td>'.esc_html($r['name']).'</td><td><a href="'.esc_url($r['source_url']).'" target="_blank" rel="noopener">'.esc_html($r['source_url']).'</a></td><td>'.esc_html($r['source_type']).'</td><td>'.((int)$r['is_active']?'attiva':'disattiva').'</td><td>'.esc_html($r['updated_at']).'</td><td>Nessun dato</td><td><button class="button button-small" disabled>Modifica</button> <button class="button button-small" disabled>Disabilita</button> <button class="button button-small" disabled>Elimina</button></td></tr>'; }
        echo '</tbody></table>';
    }
