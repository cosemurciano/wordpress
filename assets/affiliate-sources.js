jQuery(function($){
  ...
  function gygRenderActivityProgress($row, d){ if(!d){ return; } var total=parseInt(d.total_records||0,10), processed=parseInt(d.processed_records||0,10), pct=parseFloat(d.percent||0); if(isNaN(pct)){ pct=0; } var label=gygJobStatusLabel(d.status||'not_started'); $row.removeClass('status-queued status-running status-paused status-completed status-failed status-cancelled status-not_started no-mapping').addClass('status-'+(d.status||'not_started')); $row.find('.alma-gyg-progress-cell').attr('data-status', d.status||'').find('.status-label').text(label); $row.find('.alma-gyg-mini-progress span').css('width', Math.max(0, Math.min(100, pct))+'%'); $row.find('.counts').text('Processati '+processed+' / '+total+' · importati '+(d.imported_count||0)+' · aggiornati '+(d.updated_count||0)+' · già presenti '+(d.existing_count||0)+' · saltati '+(d.skipped_count||0)+' · errori '+(d.error_count||0)); $row.find('.updated').text(d.updated_at ? 'Ultimo aggiornamento: '+d.updated_at : ''); if(d.message){ $row.find('.row-message').text(d.message); } }
