jQuery(function($){
  ...
  function gygJobStatusLabel(status, label){
    var map = {not_started:'Non importato', queued:'In coda', running:'In corso', paused:'In pausa', completed:'Completato', failed:'Errore', cancelled:'Annullato', ready:'Pronto'};
  ...
  function gygActivityPayload($row, action){ return {action:action, nonce:gygNonce(), source_id:$('.alma-gyg-activity-summary').data('source-id'), token:$('.alma-gyg-activity-summary').data('token'), activity_type:$row.data('activity-type'), activity_type_hash:$row.data('activity-hash'), term_ids:gygActivityTerms($row)}; }
  function gygRenderActivityProgress($row, d){
    ...
    $row.removeClass('status-queued status-running status-paused status-completed status-failed status-cancelled status-not_started').addClass('status-'+status);
    ...
    $cell.find('.counts').text('Processati: '+processed+'/'+total+' · importati '+(d.imported_count||0)+' · aggiornati '+(d.updated_count||0)+' · già presenti '+(d.existing_count||0)+' · saltati '+(d.skipped_count||0)+' · errori '+(d.error_count||0));
  ...
  function gygPollActivityRows(){ var active=false; $('.alma-gyg-activity-row').each(function(){ var $row=$(this); $.post(gygAjaxUrl(), gygActivityPayload($row, 'alma_gyg_csv_get_activity_progress')).done(function(res){ if(res && res.success){ gygRenderActivityProgress($row, res.data); if($.inArray(res.data.status, ['queued','running','paused']) !== -1){ active=true; } gygRefreshSessionLogs(); } }); }); if($('.alma-gyg-activity-summary').length){ setTimeout(gygPollActivityRows, active ? 4000 : 9000); } }
  ...
  $(document).on('click', '.alma-gyg-start-activity-import', function(e){ e.preventDefault(); var $btn=$(this), $row=$btn.closest('.alma-gyg-activity-row'), terms=gygActivityTerms($row); if(!terms.length){ $row.find('.row-message').text('Seleziona almeno una Tipologia Link affiliati prima di importare.'); return; } $btn.prop('disabled', true).text('Avvio…'); $row.find('.row-message').text('Avvio o ripresa job background…'); $.post(gygAjaxUrl(), gygActivityPayload($row, 'alma_gyg_csv_start_activity_import')).done(function(res){ if(!res || !res.success){ $row.find('.row-message').text((res && res.data && res.data.message) || 'Errore avvio importazione.'); return; } gygRenderActivityProgress($row, res.data); gygRefreshSessionLogs(); }).fail(function(xhr){ $row.find('.row-message').text(gygAjaxErrorMessage(xhr, 'Errore di rete durante l’avvio importazione.')); }).always(function(){ $btn.prop('disabled', false).text('Importa/continua'); }); });
  if($('.alma-gyg-activity-summary').length){ setTimeout(gygPollActivityRows, 1500); }
