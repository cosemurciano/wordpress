jQuery(function($){
  ...
  function gygSetProgress(processed, requested, status){ var pct = requested ? Math.min(100, Math.round(processed / requested * 100)) : 0; var $wrap = $('.alma-gyg-progress-wrap').first(); $wrap.find('.alma-progress-bar').css('width', pct+'%'); $wrap.find('.alma-gyg-progress-status').text(status + ' ' + processed + ' / ' + requested + ' (' + pct + '%)'); }
  ...
  $(document).on('click','.alma-gyg-open-import',function(e){
    ...
    $('.alma-gyg-report').hide().empty(); $('.alma-gyg-log').html('<p class="description">Nessun errore o warning.</p>'); $('.alma-gyg-import-more').hide(); $('.alma-gyg-progress-wrap .alma-progress-bar').css('width','0%'); $('.alma-gyg-progress-wrap').hide(); $('.alma-gyg-diagnostics').remove();
  ...
  function gygJobStatusLabel(status, label){
    var map = {not_started:'Non importato', queued:'In coda', running:'In corso', paused:'In pausa', completed:'Completato', failed:'Errore', cancelled:'Annullato', ready:'Pronto', processing:'In elaborazione', partial:'Parziale'};
  ...
  function gygRenderJobStatus(d){
    ...
    $('.alma-gyg-job-status').text(gygJobStatusLabel(d.status, d.status_label)+' · '+pct+'% · '+(d.message || ''));
  ...
  $(document).on('submit', '.alma-gyg-selective-import-form', function(e){
    ...
    $('.alma-gyg-job-box').show(); $('.alma-gyg-job-status').text('Preparazione importazione manuale…');
  ...
  function gygActivityTerms($row){ var ids=[]; $row.find('.alma-gyg-activity-mapping option:selected').each(function(){ var v=parseInt($(this).val(),10); if(v>0){ ids.push(v); } }); return ids; }
  function gygActivityPayload($row, action){ return {action:action, nonce:gygNonce(), source_id:$('.alma-gyg-activity-summary').data('source-id'), token:$('.alma-gyg-activity-summary').data('token'), activity_type:$row.data('activity-type'), activity_type_hash:$row.data('activity-hash'), term_ids:gygActivityTerms($row), batch_size:$row.find('.alma-gyg-batch-size').val() || 50}; }
  function gygRenderActivityProgress($row, d){
    if(!d){ return; }
    var hash = String(d.activity_type_hash || '');
    if(hash){
      var $target = $('.alma-gyg-activity-row').filter(function(){ return String($(this).data('activity-hash') || '') === hash; });
      if($target.length){ $row = $target.first(); }
    }
    var total=parseInt(d.total_records||0,10), processed=parseInt(d.processed_records||0,10), pct=parseFloat(d.percent||0), status=String(d.status||'not_started');
    if(isNaN(pct)){ pct=0; }
    pct = Math.max(0, Math.min(100, pct));
    var label=gygJobStatusLabel(status, d.status_label);
    $row.removeClass('status-queued status-running status-paused status-completed status-failed status-cancelled status-not_started status-ready status-processing status-partial').addClass('status-'+status);
    var $cell = $row.find('.alma-gyg-progress-cell').first();
    $cell.attr('data-status', status).find('.status-label').text(label);
    var $bar = $cell.find('.alma-gyg-mini-progress').first();
    if(status === 'not_started' && processed < 1){
      $bar.remove();
    } else {
      if(!$bar.length){ $bar = $('<div/>', {'class':'alma-gyg-mini-progress', role:'progressbar', 'aria-valuemin':'0', 'aria-valuemax':'100'}).append($('<span/>')).insertAfter($cell.find('.status-label').first()); }
      $bar.attr('aria-valuenow', pct).find('span').css('width', pct+'%');
    }
    $cell.find('.counts').text('Processati: '+processed+'/'+total+' · importati '+(d.imported_count||0)+' · aggiornati '+(d.updated_count||0)+' · già presenti '+(d.existing_count||0)+' · saltati '+(d.skipped_count||0)+' · URL non validi '+(d.invalid_urls_count||0)+' · incompleti '+(d.incomplete_records_count||0)+' · errori '+(d.error_count||0)+' · non processati '+(d.unprocessed_records||0));
    $cell.find('.updated').text(d.updated_at ? 'Ultimo aggiornamento: '+d.updated_at : '');
    if(d.message){ $row.find('.row-message').text(d.message); }
  }
  function gygRenderSessionLogs(logs){ var $tbody=$('.alma-gyg-session-logs tbody'); if(!$tbody.length){ return; } $tbody.empty(); if(!logs || !logs.length){ $tbody.append('<tr><td colspan="4">Nessun evento registrato per questa sessione.</td></tr>'); return; } $.each(logs, function(_, log){ $tbody.append('<tr><td>'+gygEsc(log.created_at||'')+'</td><td>'+gygEsc(log.activity_type||'')+'</td><td>'+gygEsc(log.event||'')+'</td><td>'+gygEsc(log.message||'')+'</td></tr>'); }); }
  function gygRefreshSessionLogs(){ var $box=$('.alma-gyg-session-logs'); if(!$box.length){ return; } $.post(gygAjaxUrl(), {action:'alma_gyg_csv_get_session_logs', nonce:gygNonce(), session_id:$box.data('session-id'), source_id:$box.data('source-id')}).done(function(res){ if(res && res.success){ gygRenderSessionLogs(res.data.logs||[]); } }); }
  function gygRefreshActivityRows(){ $('.alma-gyg-activity-row').each(function(){ var $row=$(this); $.post(gygAjaxUrl(), gygActivityPayload($row, 'alma_gyg_csv_get_activity_progress')).done(function(res){ if(res && res.success){ gygRenderActivityProgress($row, res.data); } }); }); gygRefreshSessionLogs(); }
  $(document).on('change', '.alma-gyg-activity-mapping', function(){ var $row=$(this).closest('.alma-gyg-activity-row'); $row.find('.row-message').text('Salvataggio mapping…'); $.post(gygAjaxUrl(), gygActivityPayload($row, 'alma_gyg_csv_save_activity_mapping')).done(function(res){ $row.find('.row-message').text((res && res.data && res.data.message) || 'Mapping salvato.'); gygRefreshSessionLogs(); }).fail(function(xhr){ $row.find('.row-message').text(gygAjaxErrorMessage(xhr, 'Errore salvataggio mapping.')); }); });
  function gygRenderBatchReport($row, d){ var b=(d&&d.batch_report)||{}, errors=b.error_details||[], msg=(d&&d.message)||'Batch completato.'; var html='<div class="alma-gyg-batch-report notice notice-info inline"><p><strong>Report batch:</strong> '+gygEsc(msg)+'</p><ul><li>Batch size: '+gygEsc(b.batch_size||$row.find('.alma-gyg-batch-size').val()||'')+'</li><li>Range processato: '+gygEsc((b.range&&b.range.from)||'')+'–'+gygEsc((b.range&&b.range.to)||'')+'</li><li>Importati '+(b.imported||0)+' · aggiornati '+(b.updated||0)+' · saltati '+(b.skipped||0)+' · duplicati/già presenti '+(b.existing||0)+' · URL non validi '+(b.invalid_urls||0)+' · incompleti '+(b.incomplete_records||0)+' · errori '+(b.errors||0)+'</li><li>Record rimanenti: '+(d.remaining_records||0)+' · completamento '+(d.percent||0)+'%</li></ul>'; if(errors.length){ html+='<p><strong>Dettaglio errori:</strong></p><ol>'; $.each(errors.slice(0,10), function(_, er){ html+='<li>Riga/indice '+gygEsc(er.line||'')+': '+gygEsc(er.title||'(senza titolo)')+' — '+gygEsc(er.reason||'Errore')+'. '+gygEsc(er.suggested_action||'Verifica il record e riprova.')+'</li>'; }); html+='</ol>'; } html+='<p class="description">Suggerimento: se trovi molti URL non validi, correggi il CSV e usa Reset import prima di ricaricarlo.</p></div>'; $row.find('.row-message').html(html); }
  $(document).on('click', '.alma-gyg-start-activity-import', function(e){ e.preventDefault(); var $btn=$(this), $row=$btn.closest('.alma-gyg-activity-row'), terms=gygActivityTerms($row); if(!terms.length){ $row.find('.row-message').text('Seleziona almeno una Tipologia Link affiliati prima di importare.'); return; } $btn.prop('disabled', true).text('Importazione…'); $row.find('.row-message').text('Batch manuale in corso: attendi il report prima di cliccare di nuovo.'); $.post(gygAjaxUrl(), gygActivityPayload($row, 'alma_gyg_csv_process_manual_batch')).done(function(res){ if(!res || !res.success){ $row.find('.row-message').text((res && res.data && res.data.message) || 'Errore durante il batch manuale.'); return; } gygRenderActivityProgress($row, res.data); gygRenderBatchReport($row, res.data); gygRefreshSessionLogs(); }).fail(function(xhr){ $row.find('.row-message').text(gygAjaxErrorMessage(xhr, 'Il batch non è stato completato. Verifica la connessione e riprova: nessun auto-processing è in esecuzione.')); }).always(function(){ $btn.prop('disabled', false).text('Importa prossimo batch'); }); });
  $(document).on('click', '.alma-gyg-retry-errors', function(e){ e.preventDefault(); $(this).closest('.alma-gyg-activity-row').find('.row-message').text('Riprova errori non è ancora disponibile: correggi il CSV e usa Reset import se devi rielaborare le righe in errore.'); });
  if($('.alma-gyg-activity-summary').length){ gygRefreshActivityRows(); }
