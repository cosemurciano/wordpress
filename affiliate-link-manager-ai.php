...
class AffiliateManagerAI {
    ...
    public function display_affiliate_link($atts) {
        ...
        // Contenuto interno al link (immagine + eventuale titolo)
        $link_content = '';
        // Contenuto dopo il link (es. descrizione)
        $after_link = '';

        if ($atts['img'] === 'yes') {
            $link_content = get_the_post_thumbnail($atts['id'], 'full', array('class' => 'alma-affiliate-img'));
            if (!$link_content) {
                $atts['img'] = 'no';
            } else {
                if (in_array('title', $fields)) {
                    ...
                    if (in_array('content', $fields)) {
                        $link_content .= '<h4 class="alma-link-title">' . $title_html . '</h4>';
                    } else {
                        $link_content .= '<span class="alma-link-title">' . $title_html . '</span>';
                    }
                }
                if (in_array('content', $fields)) {
                    $post_content = apply_filters('the_content', get_post_field('post_content', $atts['id']));
                    $after_link = '<div class="alma-link-content">' . $post_content . '</div>';
                }
            }
        }
        ...
        if ($atts['img'] !== 'yes') {
            ...
            $link_content = esc_html($atts['text']);
        }
        ...
        $link_html .= '>' . $link_content . '</a>' . $after_link;
