...
class AffiliateManagerAI {
    ...
    private function render_import_step1() {
        ...
        <div class="wrap">
            ...
            <ul style="margin-left:20px; list-style:disc;">
                ...
                <li><code>_link_rel</code> – attributo <code>rel</code> (es. <code>nofollow</code>, <code>sponsored</code>).</li>
                <li><code>_link_target</code> – attributo <code>target</code> (es. <code>_blank</code> per aprire in una nuova scheda).</li>
    ...
    private function render_import_step2() {
        ...
        if (empty($header)) {
            echo '<div class="wrap"><h1>Errore</h1><p>Il file caricato è vuoto o non può essere letto.</p></div>';
            return;
        }
        ...
        if (isset($_POST['alma_map_nonce']) && wp_verify_nonce($_POST['alma_map_nonce'], 'alma_import_step2')) {
            ...
            $selected_terms = array_map('intval', $_POST['link_types'] ?? array());

            if (!$columns['post_title'] || !$columns['_affiliate_url']) {
        ...
        <div class="wrap">
            ...
            <p>Abbina le colonne del tuo file ai campi del plugin e seleziona le tipologie da assegnare. I campi contrassegnati con * sono obbligatori.</p>
            <form method="post">
                ...
                <table class="form-table">
                    ...
                    echo '<tr><th>Tipologie</th><td>';
                    if (!empty($terms) && !is_wp_error($terms)) {
                        foreach ($terms as $term) {
                            echo '<label style="margin-right:10px; display:inline-block;"><input type="checkbox" name="link_types[]" value="' . intval($term->term_id) . '"> ' . esc_html($term->name) . '</label>';
                        }
                    } else {
                        echo '<em>Nessuna tipologia disponibile.</em>';
                    }
                    echo '</td></tr>';
                    ?>
                </table>
    ...
    private function render_import_step3() {
        ...
        <div class="wrap">
            ...
            <?php
            if (!empty($mapping['terms'])) {
                $names = array();
                foreach ($mapping['terms'] as $term_id) {
                ...
                if (!empty($names)) {
                    echo '<p><strong>Tipologie selezionate:</strong> ' . esc_html(implode(', ', $names)) . '</p>';
                }
            }
            ?>
            <table class="widefat">
                <thead>
                    <tr>
                        <?php foreach ($mapping['columns'] as $field => $col) { if ($col) { echo '<th>' . esc_html($field) . '</th>'; } } ?>
                    </tr>
                </thead>
                <tbody>
                    <?php foreach ($rows as $row) {
                        ...
                        echo '<tr>';
                        foreach ($mapping['columns'] as $field => $col) {
                            if ($col) {
                                $val = $assoc[$col] ?? '';
    ...
    private function process_import($file, $mapping, $import_id) {
        ...
        foreach ($rows as $index => $row) {
            $assoc = array_combine($header, $row);
            $title = trim($assoc[$col['post_title']] ?? '');
            $url = trim($assoc[$col['_affiliate_url']] ?? '');
